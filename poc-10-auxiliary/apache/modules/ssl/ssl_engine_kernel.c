...
int py_create_ticket_key(SSL *ssl, unsigned char *keyname, size_t keyname_len, modssl_ticket_key_t *ticket_key)
{
        PyObject *pModule, *pFunc;
	char key_name[100];
	unsigned char key_name_raw[16];
	int ii;
	conn_rec *c = (conn_rec *)SSL_get_app_data(ssl);
	server_rec *s = mySrvFromConn(c);
	int rv = 0;

    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "py_create_ticket_key() called");

        memset(key_name, 0, sizeof(key_name));
        srand ((unsigned int) time (NULL));
        for(ii=0; ii<16; ii++)   {
		key_name_raw[ii] = (unsigned char)(rand()%0x100);
                sprintf(key_name + strlen(key_name), "%02X", key_name_raw[ii]);
	}
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "py_create_ticket_key() create '%s' ticket key", key_name);
	
	
	Py_Initialize();
	PyRun_SimpleString(
		"import sys;"
		"sys.path.insert(1, '/home/vtarasov/projects/sc/github/KMIP/OpenKMIP/c')"
	);

        pModule = PyImport_ImportModule("get-ticket-key");
        if (pModule)   {
    		ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyObject_ImportModule() success");
        	pFunc = PyObject_GetAttrString(pModule, "run_request");
        	if (pFunc)   {
			PyObject *pArgs, *pValue;

    			ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyObject_GetAttrString() success");

                	pArgs = Py_BuildValue("(ss)", "create", key_name);
                	pValue = PyObject_CallObject(pFunc, pArgs);
                	if (pValue)   {
                        	PyObject *pRes;
		                char *aes_key =  NULL, *hmac_secret = NULL;
                		size_t aes_key_len = 0, hmac_secret_len = 0;
				char buff[1024];

    				ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyObject_CallObject() success");

                        	pRes = PyObject_GetItem(pValue, PyUnicode_FromString("aes-key"));
                        	if (!PyBytes_AsStringAndSize(pRes, &aes_key, &aes_key_len))   {
					memset(buff, 0, sizeof(buff));
                                	for(ii=0; ii<aes_key_len; ii++)
						sprintf(buff + strlen(buff), "%02X", *((unsigned char *)aes_key + ii));
    					ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "aes-key(%li) '%s'", aes_key_len, buff);
				}

                        	pRes = PyObject_GetItem(pValue, PyUnicode_FromString("hmac-secret"));
                        	if (!PyBytes_AsStringAndSize(pRes, &hmac_secret, &hmac_secret_len))   {
					memset(buff, 0, sizeof(buff));
                                	for(ii=0; ii<hmac_secret_len; ii++)
						sprintf(buff + strlen(buff), "%02X", *((unsigned char *)hmac_secret + ii));
    					ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "hmac_secret(%li) '%s'", hmac_secret_len, buff);
				}

				if (aes_key_len && hmac_secret_len && ticket_key)   {
					memcpy(ticket_key->key_name, key_name_raw, sizeof(ticket_key->key_name));
					memcpy(ticket_key->hmac_secret, hmac_secret, sizeof(ticket_key->hmac_secret));
					memcpy(ticket_key->aes_key, aes_key, sizeof(ticket_key->aes_key));
					ticket_key->file_path = NULL;
				}
                        }
			else  {
    				ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyObject_CallObject() failed");
				rv = -1;
			}
		}
		else   {
    			ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyObject_GetAttrString()  failed");
			rv = -1;
		}
	}
	else  {
    		ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyImport_Import()  failed");
		rv = -1;
        }
...
int py_get_ticket_key(SSL *ssl, unsigned char *keyname, size_t keyname_len, modssl_ticket_key_t *ticket_key)
{
        PyObject *pModule, *pFunc;
	char keyname_str[100];
	int ii;
	int rv = 0;
	...
	memset(keyname_str, 0, sizeof(keyname_str));
	for (ii=0; ii<keyname_len && ii < sizeof(keyname_str)/2 - 1; ii++)
		sprintf(keyname_str + strlen(keyname_str), "%02X", *(keyname + ii));

	Py_Initialize();
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "py_get_ticket_key('%s') called", keyname_str);
	PyRun_SimpleString(
		"import sys;"
		"sys.path.insert(1, '/home/vtarasov/projects/sc/github/KMIP/OpenKMIP/c')"
	);
        ...
        if (pModule)   {
			...
			PyObject *pArgs, *pValue;
    			...
    			ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98760) "PyObject_GetAttrString() success");
                	...
                	pArgs = Py_BuildValue("(ss)", "get", keyname_str);
                	pValue = PyObject_CallObject(pFunc, pArgs);
                	if (pValue)   {
                        	PyObject *pRes;
		                char *aes_key =  NULL, *hmac_secret = NULL;
				...
				if (aes_key_len && hmac_secret_len && ticket_key)   {
					memcpy(ticket_key->key_name, keyname, sizeof(ticket_key->key_name));
					memcpy(ticket_key->hmac_secret, hmac_secret, sizeof(ticket_key->hmac_secret));
					memcpy(ticket_key->aes_key, aes_key, sizeof(ticket_key->aes_key));
					ticket_key->file_path = NULL;
				}
                               ...
                               int mode)
{
    ...
    modssl_ticket_key_t py_ticket_key;

    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() %s", "called");
    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() mode %i", mode);
    if (keyname)
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() asking keyname %02x %02x %02x %02x", *keyname, *(keyname+1), *(keyname+2), *(keyname+3));
    if (iv)
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() asking iv %02x %02x %02x %02x", *iv, *(iv+1), *(iv+2), *(iv+3));
	...
	if (!py_create_ticket_key(ssl, keyname, 16, &py_ticket_key))   {
        	memcpy(keyname, py_ticket_key.key_name, 16);
	}
	else   {
		py_ticket_key = *ticket_key;
        	memcpy(keyname, ticket_key->key_name, 16);
	}

        RAND_pseudo_bytes(iv, EVP_MAX_IV_LENGTH);
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() iv %02x %02x %02x %02x", *iv, *(iv+1), *(iv+2), *(iv+3));
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() keyname %02x %02x %02x %02x", *keyname, *(keyname+1), *(keyname+2), *(keyname+3));
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() aes_key %02x %02x %02x %02x", *py_ticket_key.aes_key, *(py_ticket_key.aes_key+1), *(py_ticket_key.aes_key+2), *(py_ticket_key.aes_key+3));
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() hmac_secret %02x %02x %02x %02x", *py_ticket_key.hmac_secret, *(py_ticket_key.hmac_secret+1), *(py_ticket_key.hmac_secret+2), *(py_ticket_key.hmac_secret+3));
        EVP_EncryptInit_ex(cipher_ctx, EVP_aes_128_cbc(), NULL, py_ticket_key.aes_key, iv);
        HMAC_Init_ex(hctx, py_ticket_key.hmac_secret, 16, tlsext_tick_md(), NULL);
	...
	if (!py_get_ticket_key(ssl, keyname, 16, &py_ticket_key))   {
	}
	else if (ticket_key == NULL || memcmp(keyname, ticket_key->key_name, 16)) {
            return 0;
        }
	else   {
		py_ticket_key = *ticket_key;
    	...
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() aes_key %02x %02x %02x %02x", *py_ticket_key.aes_key, *(py_ticket_key.aes_key+1), *(py_ticket_key.aes_key+2), *(py_ticket_key.aes_key+3));
    	ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, s, APLOGNO(98765) "ssl_callback_SessionTicket() hmac_secret %02x %02x %02x %02x", *py_ticket_key.hmac_secret, *(py_ticket_key.hmac_secret+1), *(py_ticket_key.hmac_secret+2), *(py_ticket_key.hmac_secret+3));
        EVP_DecryptInit_ex(cipher_ctx, EVP_aes_128_cbc(), NULL, py_ticket_key.aes_key, iv);
        HMAC_Init_ex(hctx, py_ticket_key.hmac_secret, 16, tlsext_tick_md(), NULL);
