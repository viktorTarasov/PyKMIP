...
class KMIPImpl(KMIP):
    ...
    def discover_versions(self, protocol_versions=None):
        self.logger.warning("discover_versions(protocol_versions={0}) called"
                .format(protocol_versions))
        ...
        self.logger.debug(msg)
        try:
            return DiscoverVersionsResult(ResultStatus(RS.SUCCESS), protocol_versions=result_versions)
        except NotImplementedError:
