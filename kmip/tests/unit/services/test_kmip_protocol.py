...
class TestKMIPProtocol(TestCase):
    ...
    def test_IO_read(self):
        ...
        protocol.socket.recv = MagicMock(
            side_effect=[self.response[:8], self.response[8:]])

        received = protocol.read()

        protocol.socket.recv.assert_any_call(8)
        protocol.socket.recv.assert_any_call(len(self.response) - 8)
    ...
    def test_IO_read_EOF(self):
        protocol = self.factory.getProtocol(MagicMock())
        protocol.socket = MagicMock()
        protocol.socket.recv = MagicMock(side_effect=[[]])
    ...
    def test_IO_read_request_length_mismatch(self):
        ...
        protocol.socket.recv.assert_has_calls(calls)
